...
conn = sqlite3.connect("bot_registaer.db", check_same_thread=False)
cursor = conn.cursor()
...
async def start_command(message: types.Message):
    ...
    await message.answer("Здравствуйте и добро пожаловать в бот!")
...
async def mailing_command(message: types.Message):
    ...
    if not is_admin or not is_admin[0]:
        await message.answer("К сожелению у вас нет прав для этой команды.")
        return
    ...
    for user in users:
        try:
            await bot.send_message(user[0], "Это ссылка от админа!")
        except Exception:
            continue

    await message.answer("Ссылка завершена.")
